# ============================================================
# monitor.py — Live terminal dashboard for OCT FL benchmark
...
def get_gs_site_status(run_name: str, site_name: str) -> tuple:
    ...
    if site_name == "test":
        out = ROOT / "data_oct" / "gs" / run_name / "test" / "images.npy"
    else:
        i   = site_name.split("-")[1]
        out = ROOT / "data_oct" / "gs" / run_name / f"site_{i}" / "images.npy"
    if out.exists():
...
def render():
    ...
    print(clr("  OCT FL Benchmark Monitor".center(width), BOLD + BLUE))
